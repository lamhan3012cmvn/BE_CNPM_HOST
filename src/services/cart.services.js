...
const createNewCart = async body => {
	try {
		...
		if (!existCart) {
			...
			const someProduct = existCart.products.findIndex(
				element => element.idProduct === idProduct
			);
			if (someProduct === -1) {
				const product = {
					idProduct: idProduct,
...
const getCarts = async body => {
	try {
		...
		await Promise.all(
			returnData.products.map(async elm => {
...
const deleteCart = async body => {
	try {
		const { decodeToken, idProduct } = body;
		const existCategory = await CART.findOne({ FK_CreateAt: decodeToken.data });
		if (!existCategory) {
			return {
		...
		existCategory.products = existCategory.products.filter(p => {
